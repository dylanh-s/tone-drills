...
import { buildLuciaOptions, buildQuestions, judge, pickVoice, questionClip, toneDigits, } from "./logic.js";
...
export class DrillApp {
    ...
    // -- rendering --------------------------------------------------------
    renderAll() {
        this.root.replaceChildren(this.renderHeader(), this.renderBody(), this.renderFooter());
    }
    grid(rows) {
        const g = el("div", "grid");
    ...
    luciaView(q, answered) {
        ...
        q.options.forEach((opt, i) => {
            ...
            if (!answered) {
                ...
                content.append(el("span", `mark ${cls}`, mark), el("span", cls, opt.word), el("span", `${cls} dim-pair`, opt.pair));
    ...
    verdictLines(q) {
        ...
        if (result === "correct")
            lines.push(el("div", "ok bold", "✓ correct"));
        else if (result === "sandhi")
            lines.push(el("div", "warn bold", "≈ sounds right, but not counted"));
        else
            lines.push(el("div", "bad bold", "✗ wrong"));
        const ya = el("div", "reveal");
        ya.append(el("span", "dim", "you: "), el("span", "", q.userAnswer ?? ""), el("span", "dim", "     answer: "), el("span", "ok bold", q.answer));
        lines.push(ya);
    ...
    renderResults() {
        ...
        const inner = el("div", "panel-body");
        inner.append(el("div", "accent bold big2", "drill complete"), el("div", "ok bold huge", `${correct} / ${total}`), el("div", "dim", `${pct.toFixed(0)}%`));
        const tally = el("div", "tally");
        tally.append(tallyRow(String(correct), "ok", "correct"));
        if (sandhi)
            tally.append(tallyRow(String(sandhi), "warn", "sounded right (sandhi, not counted)"));
        ...
        if (missed.length) {
            ...
            for (const x of missed) {
                ...
                r.append(el("span", "", x.word), el("span", "bad right", x.userAnswer || "—"), el("span", "dim", "→"), el("span", "ok bold", x.answer));
                mg.append(r);
            }
            inner.append(mg);
        }
    ...
    // -- toggles ----------------------------------------------------------
    toggleChars() { this.showChars = !this.showChars; this.renderAll(); }
    toggleOptChars() { this.showOptChars = !this.showOptChars; this.renderAll(); }
    toggleHelp() { this.showHelp = !this.showHelp; this.renderAll(); }
    toggleMode() {
    ...
    playOption(i, frame, select = false) {
        ...
        if (i >= q.options.length)
            return;
        if (select && q.userAnswer === null)
            this.selected = i;
        this.requestPlay(q.options[i].word, frame);
    }
    submitLucia() {
        const q = this.q;
        if (!q.options.length || q.userAnswer !== null)
            return;
